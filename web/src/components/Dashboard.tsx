    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div className="grid-2">
        <div className="card">
          ...
          <FleetMap robots={robots} selected={selectedRobot} onSelect={setSelectedRobot} />
        </div>
      ...
      {/* Sensor Visualizations (when robot selected) */}
      {selected && (
        <div className="grid-2">
          <JointVisualizer data={selected} />
...
function FleetMap({ robots, selected, onSelect }: {
              ...
              {group.joints.map((name) => {
                ...
                return (
                  <div key={name} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11 }}>
                    <span style={{ fontFamily: 'var(--font-mono)', color: 'var(--text-muted)', width: 140, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flexShrink: 0 }}>
