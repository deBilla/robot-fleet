...
type AnalyticsService interface {
	GetFleetAnalytics(ctx context.Context, from, to time.Time) ([]store.FleetHourlyMetric, error)
	GetRobotAnalytics(ctx context.Context, robotID string, from, to time.Time) ([]store.RobotHourlyMetric, error)
	GetAnomalies(ctx context.Context, from, to time.Time) ([]store.RobotAnomaly, error)
}
...
func (s *analyticsService) GetFleetAnalytics(ctx context.Context, from, to time.Time) ([]store.FleetHourlyMetric, error) {
	cacheKey := fmt.Sprintf("analytics:fleet:%s:%s", from.Format("2006-01-02"), to.Format("2006-01-02"))
...
func (s *analyticsService) GetAnomalies(ctx context.Context, from, to time.Time) ([]store.RobotAnomaly, error) {
	cacheKey := fmt.Sprintf("analytics:anomalies:%s:%s", from.Format("2006-01-02"), to.Format("2006-01-02"))
