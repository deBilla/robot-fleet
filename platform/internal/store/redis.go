...
// RedisClient returns the underlying Redis client for direct access (e.g., Temporal bridge).
...
// CommandDedupTTL is the window during which duplicate commands are rejected.
const CommandDedupTTL = 5 * time.Minute

// CheckCommandDedup checks if a command with the given dedup key already exists.
// Returns the existing command ID if duplicate, or 0 if new.
func (s *RedisStore) CheckCommandDedup(ctx context.Context, dedupKey string) (int64, error) {
	val, err := s.client.Get(ctx, dedupKey).Int64()
	if err == redis.Nil {
		return 0, nil
	}
	if err != nil {
		return 0, fmt.Errorf("check command dedup: %w", err)
	}
	return val, nil
}

// SetCommandDedup stores a command dedup entry with a TTL.
func (s *RedisStore) SetCommandDedup(ctx context.Context, dedupKey string, commandID int64) error {
	return s.client.Set(ctx, dedupKey, commandID, CommandDedupTTL).Err()
}
